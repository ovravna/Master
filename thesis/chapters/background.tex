...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\textwidth]{fig/hololens2}
\end{figure}
...
Git is design for projects with only (mostly) small, human-readable text files, like a code base. A Unity project often as huge files, which Git does not support, and relevant changes can happen in binary files, which makes merging impossible. Git LFS, gitignore and setting in Unity to disable binary files...
...
ITK-Snap is a interactive computer application for navigating medical imagery. It is a convectional tool used to display, segment and label 3D neuroanatomical data. It supports a wide array of visual tools which helps in learning about the brain.

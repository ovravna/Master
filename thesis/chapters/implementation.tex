...
\begin{lstlisting}[language=c]
fixed4 frag (v2f i) {
    // point from the 2D i.uv and the uniform float VariableSlider
...
\begin{lstlisting}[language=c]
fixed4 frag (v2f i) {
    // point from product of TransformMatrix and i.uv as 4D point
    float4 uvw = mul(TransformationMatrix, float4(i.uv, 0, 1));
    // tex3D samples the 3D texture _NiftiTexture3D at point uvw
    fixed4 color = tex3D(NiftiTexture3D, uvw);
    return color;
}
\end{lstlisting}
